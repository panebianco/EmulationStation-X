    ...
    mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 2))
{
    ...
    // 🔹 Traducir texto del mensaje
    mMsg = std::make_shared<TextComponent>(mWindow, es_translate(text), Font::get(FONT_SIZE_MEDIUM), 0x777777FF, ALIGN_CENTER);
    mGrid.setEntry(mMsg, Vector2i(0, 0), false, false);

    // 🔹 Botones traducidos
    if(!name1.empty())
        mButtons.push_back(std::make_shared<ButtonComponent>(mWindow, es_translate(name1), name1, std::bind(&GuiMsgBox::deleteMeAndCall, this, func1)));
    if(!name2.empty())
        mButtons.push_back(std::make_shared<ButtonComponent>(mWindow, es_translate(name2), name2, std::bind(&GuiMsgBox::deleteMeAndCall, this, func2)));
    if(!name3.empty())
        mButtons.push_back(std::make_shared<ButtonComponent>(mWindow, es_translate(name3), name3, std::bind(&GuiMsgBox::deleteMeAndCall, this, func3)));

    // 🔹 Detectar botón acelerador según texto traducido
    if(mButtons.size() == 1)
    {
